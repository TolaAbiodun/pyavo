...
for angle in range(0, nangles):
    theta1_samp, rc_1, rc_2 = calc_theta_rc(0, 1, vp_mod, vs_mod, rho_mod)
    theta1.append(theta1_samp)
    rc_zoep_pp.append([rc_1[0, 0], rc_2[0, 0]])
...
for angle in range(0, nangles):
    ...
    z_int = int_depth(h_int=[500.0], thickness=17)
...
t = np.array(time_samples(t_min=0.0, t_max=0.5))

# Convert the layer times to a numpy array
lyr_times = np.array(lyr_times)

lyr1_indx, lyr2_indx = layer_index(lyr_times)
# print(lyr1_indx, lyr2_indx)
# print(rc_zoep_pp)
# print(f'layer times is {lyr_times}')
# print(f'z int is {z_int}')
...
# Show avo inverted values
avo_inv(rc_zoep=rc_zoep_pp, ntrc=ntrc, top=line1)

vp_dig, vs_dig, rho_dig = t_domain(t=t, vp=vp_mod, vs=vs_mod, rho=rho_mod, lyr1_index=lyr1_indx, lyr2_index=lyr2_indx)
